...
function grid:reset()
    ...
    self.rect = {
        x = self.center.x - (self.num_columns * self.cell_size) / 2,
    ...
    for i in pairs(piece.shape) do
        for j in pairs(piece.shape[i]) do
            if piece.shape[i][j] != 0 then
                ...
                if test_pos.x < 1 or test_pos.x > self.num_columns
                        or test_pos.y < 1 or test_pos.y > self.num_rows
                        or self.mtx[test_pos.y][test_pos.x] != 0 then
...
function grid:get_cell(cx, cy)
    if cx < 1 or cy < 1 or cx > self.num_columns or cy > self.num_rows then return -1 end
    -- log("returning from get_cell: " .. self.mtx[cy][cx])
    return self.mtx[cy][cx]
end

function grid:print_grid()
    for i = 1, self.num_rows do -- y
        line = ""
        for j = 1, self.num_columns do -- x
            line = line .. self.mtx[i][j]
        end
        log(line)
    end
end
...
function grid:perform_line_clears(start_row)
    ...
    local beebo = false
    local mask = { 0, 0, 0, 0 } -- for determining which rows to eradicate
    -- dont even worry about this
    local iteration = 1
    for i = start_row, max(start_row - 4, 1), -1 do
        beebo = false
        for j = 1, self.num_columns do
            log("am i going insane? cell at row " .. i .. " and column " .. j .. " is " .. self:get_cell(j, i))
            if self:get_cell(j, i) == 0 then -- empty
